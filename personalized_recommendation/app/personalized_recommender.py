...
class PersonalizedRecommender:
    def __init__(self, genai_api_key: str, model: str, news_class: NewsFetcher):
        # Constructor to initialize the recommender system.
        
        genai.configure(api_key=genai_api_key)
        self.news_fetcher = news_class
    ...
    def recommend(self, user_preferences: str, user_interactions: List[str], limit: int = 5) -> List[Dict]:
        ...
        try:
            ...
            user_embedding = 0.4 * aggregated_embedding + 0.6 * preference_embedding
           
            # Fetch all news articles
            articles = self.news_fetcher.fetch_news(limit=500)
    ...
    def recommend(self, user_preferences: str, news_ids: List[str], limit: int = 5) -> List[Dict]:
        # personalized recommendations for the user.

        # Fetch news articles
        # articles = self.news_fetcher.fetch_latest_news()
        
        articles = self.news_fetcher.fetch_news(limit=10)  # Fetch more for better filtering
        interactions = self.news_fetcher.fetch_news_by_ids(news_ids)
